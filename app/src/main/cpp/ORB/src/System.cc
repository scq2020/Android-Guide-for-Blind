    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
{
    ...
    if(mStrLoadAtlasFromFile.empty())
    {
        ...
        bool bVocLoad = mpVocabulary->loadFromBinaryFile(strVocFile);//这句话有问题错误
    ...
    else
    {
        ...
        if(!isRead)
        {
            cout << "Warning: Could not load atlas file '" << mStrLoadAtlasFromFile
                 << ".osa'. Starting with a new map." << endl;
            LOGI("Warning: Could not load atlas file. Starting with a new map.");
            // 不退出，而是从头开始初始化系统
            // 创建KeyFrame数据库
            mpKeyFrameDatabase = new KeyFrameDatabase(*mpVocabulary);
            // 创建新地图
            mpAtlas = new Atlas(0);
            loadedAtlas = false;  // 标记为未加载地图
        }
        //mpKeyFrameDatabase = new KeyFrameDatabase(*mpVocabulary);
