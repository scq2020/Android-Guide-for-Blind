...
//			if (DEBUG) Log.i(TAG, "unregister:");
		...
		if (isRegistered()) {
			if (device != null) {
				if (mUsbManager.hasPermission(device)) {
					...
					Log.v(TAG, "beforeProcessConnect" );
					processConnect(device);
				} else {
					Log.v(TAG, "requestPermission:nohasPermission");
					try {
						// パーミッションがなければ要求する
						...
						Log.w(TAG, e);
						Log.v(TAG, "requestPermission:catch:processCancel");
						processCancel(device);
						result = true;
					}
				}
			} else {
				Log.v(TAG, "requestPermission:device=null");
				processCancel(device);
				result = true;
			}
		} else {
			Log.v(TAG, "requestPermission:isnoRegistered");
			processCancel(device);
			result = true;
		}
		return result;
		...
		public void onReceive(final Context context, final Intent intent) {
			...
			if (ACTION_USB_PERMISSION.equals(action)) {
				...
				synchronized (USBMonitor.this) {
					...
					Log.v(TAG, "onReceive:device="+device);
					if (intent.getBooleanExtra(UsbManager.EXTRA_PERMISSION_GRANTED, false)) {
						if (device != null) {
							// get permission, call onConnect
							Log.v(TAG, "onReceive:processConnect");
							processConnect(device);
						}
					} else {
	...
	public static final String getDeviceKeyName(final UsbDevice device, final String serial, final boolean useNewAPI) {
		...
		if (useNewAPI && BuildCheck.isAndroid5()) {
			sb.append("#");
			if (TextUtils.isEmpty(serial)) {
				//Log.v(TAG, "serialNum:"+device.getSerialNumber());
				Log.v(TAG, "SDK_INT:"+Build.VERSION.SDK_INT);
				if (Build.VERSION.SDK_INT>27){
					sb.append((String) null);    sb.append("#");
				}else{
					Log.v(TAG, "serialNum:" + device.getSerialNumber());
